...
创建一个saver来保存模型，调用tf.train.saver()来创建一个saver，在会话中
调用saver.save()函数--第一个参数为sess的变量，第二个参数为Model报存的地址和名称
...
saver = tf.train.Saver()  # 创建一个saver保存模型
with tf.Session() as sess:
    ...
    plt.show()
    # 训练完成后保存模型
    saver.save(sess, 'model/lineRegressionModel')  # 保存为对应名称的模型
# 绘制拟合过程中误差变化曲线
